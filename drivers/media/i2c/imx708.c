...
#define IMX708_TEST_PATTERN_R_DEFAULT	IMX708_TEST_PATTERN_COLOUR_MAX
#define IMX708_TEST_PATTERN_GR_DEFAULT	0
#define IMX708_TEST_PATTERN_B_DEFAULT	0
#define IMX708_TEST_PATTERN_GB_DEFAULT	0

/*
 * Embedded metadata stream structure. Enough for 4 full resolution
 * lines of 5760 bytes (4608 pixels).
*/
#define IMX708_EMBEDDED_LINE_WIDTH (24<<10)
...
static u32 imx708_get_format_code(struct imx708 *imx708, u32 code)
{
	unsigned int i;
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->pad == IMAGE_PAD) {
		...
		if (fse->code != imx708_get_format_code(imx708, fse->code))
			return -EINVAL;
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		try_fmt->code = fmt->pad == IMAGE_PAD ?
				imx708_get_format_code(imx708, try_fmt->code) :
		...
		if (fmt->pad == IMAGE_PAD) {
			imx708_update_image_pad_format(imx708, imx708->mode,
						       fmt);
			fmt->format.code =
			       imx708_get_format_code(imx708, imx708->fmt.code);
		} else {
...
static
unsigned int imx708_get_frame_length(const struct imx708_mode *mode,
				     const struct v4l2_fract *timeperframe)
{
	...
	if (WARN_ON(frame_length > IMX708_FRAME_LENGTH_MAX))
		frame_length = IMX708_FRAME_LENGTH_MAX;
...
static void imx708_set_framing_limits(struct imx708 *imx708)
{
	...
	frm_length_min = imx708_get_frame_length(mode, &mode->timeperframe_min);
	frm_length_default =
		     imx708_get_frame_length(mode, &mode->timeperframe_default);

	/* Update limits and set FPS to default */
	__v4l2_ctrl_modify_range(imx708->vblank, frm_length_min - mode->height,
				 IMX708_FRAME_LENGTH_MAX - mode->height,
				 1, frm_length_default - mode->height);

	/* Setting this will adjust the exposure limits as well. */
	__v4l2_ctrl_s_ctrl(imx708->vblank, frm_length_default - mode->height);
...
static int imx708_init_controls(struct imx708 *imx708)
{
	...
	ret = v4l2_ctrl_new_fwnode_properties(ctrl_hdlr, &imx708_ctrl_ops,
					      &props);
	if (ret)
		goto error;
